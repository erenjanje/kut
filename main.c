...
void test_compiler() {
    const char* zort =
        "ekran degiskeni 5 olsun\n"
        "ekran (5 (3 2 *) +) {x y | x y +} yazsin\n";
    const char* endptr = zort+strlen(zort);
    KutASTNode nod = kutast_newStatement(&zort, endptr);
    kutast_debug(nod, "   ");
    kutast_destroy(nod);
    nod = kutast_newStatement(&zort, endptr);
    kutast_debug(nod, "   ");
    kutast_destroy(nod);
}
